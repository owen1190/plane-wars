...
function plan(hp,X,Y,sizeX,sizeY,score,dietime,sudu,boomimage,imagesrc){
    ...
    this.imagenode=null;
    this.planhp=hp;
    this.planscore=score;//分数
    this.plansizeX=sizeX;
    this.plansizeY=sizeY;
    this.planboomimage=boomimage;//飞机爆炸图片
    this.planisdie=false;
    this.plandietimes=0;
    this.plandietime=dietime;//死亡时间
    this.plansudu=sudu;//飞行速度
 ...
 */
    ...
    for(var i=0;i<enemyslen;i++){
        if(enemys[i].planisdie!=true){//敌机未死亡时，则飞机继续飞行
            enemys[i].planmove();
        }
/*
 如果敌机超出边界,删除敌机
